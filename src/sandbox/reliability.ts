...
function buildProbeCommand(markerToken: string, previousMarkerToken?: string): string {
  ...
  return [
    ...
    'if [ -n "$previous_process_marker" ] && command -v ps >/dev/null 2>&1; then ps_output=$(ps axww 2>/dev/null || ps auxww 2>/dev/null || true); case "$ps_output" in *"$previous_process_marker"*) process_marker_seen=true;; esac; fi',
    ...
    'printf "processIsolationOk=%s\\n" "$([ "$process_marker_seen" = false ] && printf true || printf false)"',
